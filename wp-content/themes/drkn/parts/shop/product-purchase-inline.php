...
if($is_limited) {
	...
	<div class="product-size-container">
		...
		<div class="wrap-buy-button">
			...
			<button id="js-submitPurchase" class="js-submitPurchase u-mainButton buy unlock">
			<?php echo ($is_available['info']['stockOfAllItems'] == 0 ? '<span>SOLD OUT</span>' : '<span>UNLOCK</span>'); ?>
			</button>
		</div>
		</a>
	</div>
...
if(($is_limited && $product->unlockedForPurchase()) || !$is_limited) {
	$product->renderStartPurchaseFormInline();
	?>
	<div class="product-size-container">
		<ul id="js-productSizes" class="js-productSizes product-size">
			<?php
		...
		<div class="wrap-buy-button">
			<?php $is_available = EscGeneral::isAvailable($post->ID); ?>
			<button id="js-submitPurchase" class="js-submitPurchase u-mainButton buy" data-hover-text="Choose Size" data-orig-text="BAG IT" data-co-text="Go To Checkout" type="submit"<?php echo ($is_available['info']['stockOfAllItems'] == 0 ? 'disabled="disabled"' : ''); ?>>
