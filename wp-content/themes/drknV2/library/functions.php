...
/**
 * Silk product category custom meta fields
 */
wp_enqueue_script( 'wp-color-picker' );
wp_enqueue_style( 'wp-color-picker' );

require_once realpath( dirname( __FILE__ ) ) . '/../Tax-meta-class/Tax-meta-class.php';

$silkProdCatMeta = new Tax_Meta_Class( array( 'pages' => array( 'silk_category' ), 'use_with_theme' => true ) );

$silkProdCatMeta->addCheckbox( 'is_colab', array( 'name' => __( 'Is Colab?', 'DRKN' ) ) );
...
for( $i = 1; $i <= 3; $i++ ) {
	$silkProdCatMeta->addImage( 'colab_image_' . $i, array( 'name' => __( 'Colab Image ' . $i, 'DRKN' ) ) );
	$silkProdCatMeta->addSelect( 'colab_image_pos_' . $i, $options, array( 'name' => __( 'Colab Image Position ' . $i, 'DRKN' ) ) );
	$silkProdCatMeta->addCheckbox( 'colab_image_set_' . $i, array( 'name' => __( 'Is Wide Colab Image ' . $i . '?', 'DRKN' ) ) );
}

$silkProdCatMeta->Finish();
